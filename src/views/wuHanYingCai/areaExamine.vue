  ...
  <el-tabs :ref="(dom) => (tabsRef = dom)" v-model="activeName" class="tabs">
    <el-tab-pane label="举荐单位审核" name="举荐单位审核" label-width="120px">
      <v-table
        ...
        method="post"
        :query="{ cityEnterpriseTypes: ['1', '2', '3'], districtName: onlyOrg }"
      >
        <template #query="{ query }">
          ...
          <el-form-item label="单位类型">
            <v-select
              v-model="query.chooseType"
              :options="[
                { label: '重点高校', val: '4' },
                { label: '重点实验室', val: '3' },
                { label: '企事业单位-创投机构推荐', val: '1' },
                { label: '企事业单位-非创投机构推荐', val: '2' },
              ]"
              field-value="val"
              field-label="label"
            ></v-select>
        ...
        <template #button="{ query }">
          <v-export
            ...
            filename="区级举荐单位表.xlsx"
            method="post"
            :query="{
              org: query.org,
              cityEnterpriseTypes: ['1', '2', '3'],
    ...
    >
      <v-table
        ...
        url="/talentserver/talent/backend/getByConditionQuery"
        :parse="true"
      >
        <template #query="{ query }">
          <el-form-item v-if="!isOrg" label="区/开发区">
          ...
          <el-form-item v-else label="区/开发区">
            <!-- <el-input v-model="query.districtName" disabled></el-input> -->
            <el-input v-model="onlyOrg" disabled></el-input>
          </el-form-item>
          <!-- <el-form-item label="区/开发区">
            <v-select
              v-if="isOrg == '864181926267715584'"
              v-model="query.districtName"
              :options="regionList"
            ></v-select>
            <v-select
              ...
              :options="[choseArea]"
              :lazy="true"
              field-label="dictLabel"
              field-value="dictValue"
              :placeholder="choseArea.name"
              :disabled="isOrg == '864181926267715584' ? false : true"
            ></v-select>
          </el-form-item> -->
          <el-form-item label="审核状态">
            <v-select
        ...
        <template #button="{ query }">
          <v-export
            url="/talentserver/talent/backend/export"
            filename="区级人才申报表.xlsx"
            method="post"
            :query="{
              org: query.org,
              status: query.status,
        ...
        <el-tab-pane label="申报人基本情况" name="申报人基本情况">
          ...
          <el-form-item label="审核意见" prop="status">
            <el-radio-group
              ...
              :disabled="checkIn ? false : true"
            >
              <el-radio :label="'104'">退回</el-radio>
              <el-radio :label="'2'">通过</el-radio>
...
export default defineComponent({
  ...
  setup() {
    ...
    //   set(value) {
    //     quillEditorRef.value.enable = false;
    //   },
    //   get() {
    //     return quillEditorRef.value.enable;
    //   },
    // });
    const examine = (row) => {
      state.checkIn = true;
    ...
    // 只有特定区时（一个）
    // const getArea = (param) => {
    //   http.get(`/systemserver/system/org/per`, param).then((res) => {
    //     console.log(res);
    //     if (res.data) {
    //       // state.choseArea = res.data
    //       let d = res?.data?.[0];
    //       let dd = state.areaList.find((e) => e.code == d);
    //       if (dd) {
    //         state.onlyOrg = dd.area;
    //       }
    //     }
    //   });
    // http.get(`/systemserver/system/org/${isOrg.value}`).then((res) => {
    //   if (res.data) {
    //     state.choseArea = res.data;
    //     console.log(state.choseArea);
    //   }
    // });
    // };
    // 举荐单位查看
    ...
    const examine1 = (row) => {
      ...
      http
        ...
        .then((res) => {
          if (res.data) {
            state.model = res.data;
            // quillEditorRef.value.setHTML(res.data?.work);
          }
        });
    };
    // 人才申报审核查看
