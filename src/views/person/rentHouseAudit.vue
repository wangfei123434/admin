<template>
  <!-- url="/rentserver/apartmentLeaseApply/getAll" method="get"  -->
  <V-Table
  ...
  >
    <template #query="{ query }">
      ...
      <!-- <el-form-item label="区人社审核状态" class="area">
        <el-select clearable v-model="query.peopleClubCheck" placeholder="请选择">
          <el-option
      ...
      <el-form-item label="人才等级">
        <el-select v-model="query.talentGrade" clearable placeholder="请选择">
          <el-option
            v-for="(item, index) in rcdjList"
            :key="index"
            :label="item"
            :value="item"
          ></el-option>
    ...
    <el-table-column label="操作" align="center" width="200px">
      <template #default="{ row }">
        <el-button type="text" class="check">
          ...
          <!-- 此处判断有点复杂 主status为审核中就去判断角色 -->
          <span v-if="row.status == '审核中'">
  ...
  <el-dialog v-model="showAttach" width="60%" title="附件">
    <v-table :data="attachList">
      ...
      <el-table-column label="操作">
        <template #default="{ row }">
          <v-export
            :url="`/rentserver/fileStore/review/${row.id}`"
            type="preview-target"
            file-type="image/jpeg"
          >
            <el-button v-if="row.fileType != 'zip'" type="text">预览</el-button>
            <!-- <template #preview="{ file }">{{ file }}</template> -->
          </v-export>
          <v-export
            :url="`/rentserver/fileStore/backend/downloadFile/${row.id}`"
            :filename="row.name"
            file-type="image/jpeg"
          >
    ...
    <div class="tx-box">
      ...
      >
        <tbody>
          ...
          <tr>
            <td class="infobt">人才类别</td>
            <td colspan="2">
              <span>
                {{ itemObj.talentType }}
              </span>
            </td>
            <td class="infobt">人才等级</td>
            <td colspan="2">
              <span>
                {{ itemObj.talentGrade }}
              </span>
          ...
          <!-- <tr>
          <td class="infobt" :rowspan="familyList.length * 1 + 2">
            家庭成员情况
          </td>
          <td class="infobt">婚姻状态</td>
          <td colspan="5">
            <span id="labUnit">{{ itemObj.marital }}</span>
          </td>
        </tr> -->
          <tr>
  ...
  >
    ...
    <div class="fileList">
      ...
      <el-form-item label="备注（退回/不通过原因）">
        <el-input v-model="shForm.opinion" type="textarea"></el-input>
      </el-form-item>
    </div>
  </VFormDialog>
