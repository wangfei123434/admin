<template>
  <V-Table
    ...
    :query="defaultQuery"
    url="/rentserver/apartmentLeaseApplySkillTalent/getByConditionQuery"
    :parse="true"
    label-width="100px"
  ...
  >
    ...
    <template #button="{ query }">
      <v-export
        url="/rentserver/apartmentLeaseApplySkillTalent/exportAlas"
        method="post"
        :query="{
      ...
      <template #default="{ row }">
        <el-button type="text" class="check">
          ...
          <span v-if="mask == 'ROLE_HOUSE_MANAGE'">
            ...
            <!--              审核-->
            <!--            </span>-->
            <span @click="toDetail(row.id)">查看</span>
          </span>
          <!-- 人社局角色 -->
          ...
          <span v-else>
            <span @click="toDetail(row.id)">查看</span>
            <!--            <span v-else @click="toAudit(row.id)">审核</span>-->
          </span>
        </el-button>
      ...
      >
        <tbody>
          <tr>
            ...
            <td>
              <span id="labNation">{{ itemObj.age }}</span>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td colspan="5">
              <span id="labLiveAddress">{{ itemObj.mailingAddress }}</span>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td colspan="5">
              <span>{{ itemObj.socialSecurityUnit }}</span>
            </td>
          </tr>
          <tr>
            <td class="infobt">缴费状态</td>
            <td colspan="5">
              <span>{{ itemObj.socialSecurityStatus }}</span>
            </td>
          </tr>
          <tr>
      ...
      >
        <tbody>
          ...
          <tr>
            ...
            <td class="infobt">技能等级</td>
            <td colspan="5">
              <span id="labSchool">{{ itemObj.skillGrade }}</span>
            </td>
          </tr>
  ...
  >
    <div class="fileList">
      <el-form-item label="作废原因" prop="opinion">
        <el-input v-model="zfForm.fangGuanOpinion" type="textarea"></el-input>
      </el-form-item>
    </div>
  </VFormDialog>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onSubmitInfo2 = (params, done) => {
      ...
      http
        .post(
          "/rentserver/apartmentLeaseApplySkillTalent/setAuditStatus",
          paramsObj,
          {
            message: true,
          }
