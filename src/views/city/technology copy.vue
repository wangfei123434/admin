<template>
  ...
  >
    ...
    <template #default>
      ...
      <el-table-column label="申请时间" prop="dateCreated"></el-table-column>
    ...
    <template #default="{ done }">
      <V-Table :data="fileState.list">
        ...
        <el-table-column label="操作" width="100">
          <template #default="{ row }">
            <v-export
              :url="`/talentserver/backend/attachment/reviewWord?url=${encodeURIComponent(
                row.url
              )}`"
              type="preview-target"
              file-type="text/html"
            >
              <el-button type="text" v-if="row.fileType != 'zip'">预览</el-button>
  ...
  <v-form-dialog v-model="logVisible" title="日志" :footer="null">
    <template #default="{ done }">
      <V-Table
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      ...
      queryTypes: [{ label: "战略科技人才", value: "zl" }],
    ...
    const onPersonDetailShow = (row: Record<string, any>) => {
      ...
      http
        .get(
          `/talentserver/backend/talent/district/accept/qingshan/getDetail/${row.id}`
        )
        .then((res) => {
          model.value = res.data;
        });
    ...
    const onLogSHow = (row: Record<string, any>) => {
      ...
      nextTick(() => {
        (logTableRef.value as any).onFresh();
      });
    };
